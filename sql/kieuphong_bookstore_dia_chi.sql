...
CREATE TABLE `dia_chi` (
  `ma_dia_chi` bigint NOT NULL AUTO_INCREMENT,
  `ma_nguoi_dung` bigint NOT NULL,
  `ten_dia_chi` varchar(64) DEFAULT NULL,
  `nguoi_nhan` varchar(120) DEFAULT NULL,
  `so_dien_thoai` varchar(30) DEFAULT NULL,
  `dia_chi_chi_tiet` varchar(255) DEFAULT NULL,
  `phuong_xa` varchar(128) DEFAULT NULL,
  `quan_huyen` varchar(128) DEFAULT NULL,
  `tinh_thanh` varchar(128) DEFAULT NULL,
  `ma_buu_dien` varchar(20) DEFAULT NULL,
  `mac_dinh` tinyint(1) DEFAULT '0',
  PRIMARY KEY (`ma_dia_chi`),
  KEY `fk_dc_nd` (`ma_nguoi_dung`),
  CONSTRAINT `fk_dc_nd` FOREIGN KEY (`ma_nguoi_dung`) REFERENCES `nguoi_dung` (`ma_nguoi_dung`)
...
INSERT INTO `dia_chi` VALUES (7,1,'Nhà riêng','Dung Nguyen','0936164255','Chung cư di dân X2','Nghĩa Đô','Cầu Giấy','Hà Nội','110000',1);
